...
class TestTaskFiltering:
    ...
    def setup(self, client):
        ...
        self.author = User.objects.create_user(username='author', first_name='Имя', last_name='Фамилия', password='123')
        self.executor = User.objects.create_user(username='executor', first_name='Эк', last_name='Сполнитель', password='123')
    ...
    def test_filter_by_executor(self):
        response = self.client.get(reverse('task_list'), {'executor': self.executor.pk})
        assert response.status_code == 200
        assert self.task.name in response.content.decode()
